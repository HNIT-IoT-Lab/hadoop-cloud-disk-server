...
public class RedisBloom {
    ...
    private static final RedisScript<Boolean> bfreserveScript = new DefaultRedisScript<>("if redis.call('bf.reserve', KEYS[1], ARGV[1], ARGV[2]) then return true else return false end", Boolean.class);
    ...
    public Boolean bfreserve(String key, double errorRate, int initialSize){
        ...
        try {
            create = redisTemplate.execute(bfreserveScript, Collections.singletonList(key), errorRate+"", initialSize+"");
        }catch (Exception e){
    ...
    public List<Integer> bfmadd(String key, String... values){
        return (List<Integer>)redisTemplate.execute(this.generateScript(bfmaddScript, values), Collections.singletonList(key), values);
    }
    ...
    public List<Integer> bfmexists(String key, String... values){
        return (List<Integer>)redisTemplate.execute(this.generateScript(bfmexistsScript, values), Collections.singletonList(key), values);
    }
